...
The deployment of Large Language Models (LLMs) on edge devices — local computing hardware that processes data on-device rather than relying on remote cloud infrastructure — presents substantial challenges due to their significant memory and computational requirements. Unlike cloud GPU servers that offer 40-80GB of accelerator memory, edge hardware such as Apple Silicon Macs operates under tighter memory constraints while providing the key advantages of user privacy, zero network latency, and offline capability. This paper presents a comparative study of quantization techniques — FP16 (16-bit floating point), INT8 (8-bit integer), and INT4 (4-bit integer) — applied to two representative open-source models: Phi-4 Mini (3.8B) and Mistral 7B Instruct v0.3 (7B), deployed on Apple Silicon via the CoreML framework. We benchmark each model-quantization combination across four key metrics: inference latency (tokens/second), model size on disk (MB), peak memory consumption (GB), and accuracy on a subset of the MMLU benchmark. Our results demonstrate that INT4 quantization consistently delivers the best balance between model size reduction (up to 75% compared to FP16) and acceptable accuracy degradation, making it the most viable technique for real-time LLM inference on Apple Silicon edge hardware. This study provides actionable guidance for macOS and Apple Silicon developers seeking to deploy capable language models locally on Apple Silicon hardware.
...
*\* INT4 speed advantage is hardware-dependent; dequantization overhead may reduce gains on certain Apple Silicon configurations \[9\]*

**3.1 FP16 — 16-bit Floating Point (Baseline)**

FP16, or half-precision floating point, represents each model weight using 16 bits and is the standard deployment format for CoreML models converted directly from PyTorch or Hugging Face checkpoints. It retains the full representational capacity of the original model with negligible deviation from FP32 precision, making it the natural baseline against which all other quantization methods are measured [\[4\]](#bookmark=id.jmn8z7za3j4b). For the MacBook Pro M1 Pro with 16GB unified memory used in this study, the 14GB FP16 Mistral 7B model leaves under 2GB for the operating system, making it borderline for stable inference. For the smaller Phi-4 Mini (\~7.6GB FP16), FP16 fits comfortably. FP16 is included in this comparison as the accuracy ceiling against which accuracy degradation in INT8 and INT4 is measured.
...
INT4 quantization is the most aggressive compression technique evaluated, reducing each weight to a 4-bit integer with only 16 discrete possible values. This achieves a 75% reduction in model size compared to FP16 — making it the configuration that most comfortably fits both evaluated models within the 16GB unified memory of the MacBook Pro M1 Pro, leaving ample headroom for the OS and application. Apple coremltools implements INT4 using a block-wise AWQ strategy [\[6\]](#bookmark=id.zgh5xjj11f7c), which identifies the 1% of weights most sensitive to quantization error and protects them with higher-precision scale factors before rounding. Research has shown that INT4 accuracy degradation is task-dependent: commonsense and factual recall tasks are relatively resilient, while mathematical reasoning tasks may see degradation of 2-4% on benchmarks such as MMLU [\[10\]](#bookmark=id.xdtjw22nvwvd). Despite this trade-off, INT4 represents the most viable configuration for production LLM deployment on Apple Silicon and is the focus of experimental evaluation in this paper.
...
Both models are acquired as pre-converted FP16 CoreML packages directly from the Hugging Face Hub, as manual PyTorch-to-CoreML conversion fails for these architectures due to unsupported Rotary Position Embedding (RoPE) operations in the standard coremltools tracing pipeline. Phi-4 Mini FP16 is downloaded from the community-converted AlonBBar/phi4mini-task-assistant-coreml repository. Mistral 7B FP16 is downloaded from the Apple-published apple/mistral-coreml repository [\[7\]](#bookmark=id.s0yt0cfrtyn5). Both models are used in their instruction-tuned variants to ensure comparable prompt-response behavior across evaluations.

***Stage 2 — CoreML Conversion (FP16 Baseline)***

The downloaded FP16 CoreML packages serve directly as the baseline — no additional conversion step is required. The Mistral 7B FP16 package from Apple employs a stateful KV-cache architecture introduced at WWDC 2024 [\[7\]](#bookmark=id.s0yt0cfrtyn5), where attention key-value buffers are stored as persistent CoreML state rather than passed as inputs per decoding step, significantly improving tokens-per-second throughput. The FP16 .mlpackage for each model serves as both the accuracy baseline and the input for subsequent quantization stages.
...
Since this study involves inference-only evaluation of pre-trained models rather than model training, the experimental setup focuses on systematic benchmarking. Each of the six model-quantization combinations (2 models x 3 quantization levels) is evaluated across four metrics as defined in Table 4\.
...
**\[7\]**  Hugging Face & Apple, "Running Mistral 7B with Core ML," Hugging Face Blog, 2024\. \[Online\]. Available: [https://huggingface.co/blog/mistral-coreml](https://huggingface.co/blog/mistral-coreml)

**\[8\]**  H. Xu et al., "Inference Performance Evaluation for LLMs on Edge Devices with a Novel Benchmarking Framework and Metric," arXiv:2508.11269, 2025\. \[Online\]. Available: [https://arxiv.org/abs/2508.11269](https://arxiv.org/abs/2508.11269)

**\[9\]**  T. Dettmers et al., "Profiling Large Language Model Inference on Apple Silicon: A Quantization Perspective," arXiv:2508.08531, 2025\. \[Online\]. Available: [https://arxiv.org/abs/2508.08531](https://arxiv.org/abs/2508.08531)

**\[10\]**  R. Ahmad et al., "Sustainable LLM Inference for Edge AI," ACM Transactions on Internet of Things, 2025\. \[Online\]. Available: [https://dl.acm.org/doi/10.1145/3767742](https://dl.acm.org/doi/10.1145/3767742)

**\[11\]**  Apple Machine Learning Research, "Deploying Transformers on the Apple Neural Engine," Apple ML Research Blog, 2022\. \[Online\]. Available: [https://machinelearning.apple.com/research/neural-engine-transformers](https://machinelearning.apple.com/research/neural-engine-transformers)

**\[12\]**  Microsoft Research, "Phi-4-Mini Technical Report: Compact yet Powerful Multimodal Language Models via Mixture-of-LoRAs," arXiv:2503.01743, 2025\. \[Online\]. Available: [https://arxiv.org/abs/2503.01743](https://arxiv.org/abs/2503.01743)

**\[13\]**  D. Hendrycks et al., "Measuring Massive Multitask Language Understanding," arXiv:2009.03300, 2021\. \[Online\]. Available: [https://arxiv.org/abs/2009.03300](https://arxiv.org/abs/2009.03300)
